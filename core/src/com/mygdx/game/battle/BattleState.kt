...
class BattleState : BattleSubject(), InventoryObserver {
    private var currentOpponent: EntityConfig? = null
    ...
    private fun setCurrentOpponent() {
        ...
        val entity: EntityConfig = MonsterFactory.instance?.getMonsterConfigByZoneID(currentZoneLevel) ?: return
    ...
    private val playerMagicUseCheckTimer: Timer.Task
        get() = object : Timer.Task() {
            override fun run() {
                var mpVal = ProfileManager.instance.getProperty("currentPlayerMP", Int::class.java)!!
    ...
    private val playerAttackCalculationTimer: Timer.Task
        get() = object : Timer.Task() {
            override fun run() {
                var currentOpponentHP = currentOpponent!!.getPropertyValue(EntityConfig.EntityProperties.ENTITY_HEALTH_POINTS.toString()).toInt()
                val currentOpponentDP = currentOpponent!!.getPropertyValue(EntityConfig.EntityProperties.ENTITY_DEFENSE_POINTS.toString()).toInt()
                ...
                currentOpponent!!.setPropertyValue(EntityConfig.EntityProperties.ENTITY_HEALTH_POINTS.toString(), currentOpponentHP.toString())
                Gdx.app.debug(TAG, "Player attacks " + currentOpponent!!.entityID + " leaving it with HP: " + currentOpponentHP)
                currentOpponent!!.setPropertyValue(EntityConfig.EntityProperties.ENTITY_HIT_DAMAGE_TOTAL.toString(), damage.toString())
                if (damage > 0) {
    ...
    private val opponentAttackCalculationTimer: Timer.Task
        get() = object : Timer.Task() {
            override fun run() {
                val currentOpponentHP = currentOpponent!!.getPropertyValue(EntityConfig.EntityProperties.ENTITY_HEALTH_POINTS.toString()).toInt()
                if (currentOpponentHP <= 0) {
                    //notify(currentOpponent, BattleEvent.OPPONENT_TURN_DONE)
                    return
                }
                val currentOpponentAP = currentOpponent!!.getPropertyValue(EntityConfig.EntityProperties.ENTITY_ATTACK_POINTS.toString()).toInt()
                val damage = MathUtils.clamp(currentOpponentAP - currentPlayerDP, 0, currentOpponentAP)
                var hpVal = ProfileManager.instance.getProperty("currentPlayerHP", Int::class.java)!!
                ...
                Gdx.app.debug(TAG, "Player HIT for " + damage + " BY " + currentOpponent!!.entityID + " leaving player with HP: " + hpVal)
                //notify(currentOpponent, BattleEvent.OPPONENT_TURN_DONE)
            }
        }
